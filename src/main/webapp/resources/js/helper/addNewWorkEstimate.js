...
$(document).ready(function() {
	...
	$("#basicInfoBtn").click(function() {
		...
		/*	 var editImageName = $('#imageUploadCheck').val();
				var editImagedName = $('#drawingUploadCheck').val();
				var inspecfiles=$("#uploadInspectionFiles").val();
			 if(editImageName=='')
				{
			...
			if(editImagedName=='')
			{
				alert("Please Upload Drawings");
				return false;
			}
			
			if(inspecfiles=='')
				{
				alert("Please Upload Inspection Note");
				return false;
				}
			 */
...
//												row += ' <input placeholder="" type="text" name="height" id="H'+data[i].id+'" maxlength="6" onkeypress="return isNumber(event, this)" onkeyup="calculat('+data[i].id+')">'
			    ...
			    success: function (data) {
					if (data != null) {
						...
						//	row += "<tr ondblclick='editDataAnalysisDetails(\""+data[i].id+"\",\""+data[i].rateAnalysisName+"\",\""+data[i].purpose+"\",\""+data[i].quantity+"\",\""+data[i].rate+"\",\""+data[i].unitName+"\","+id+")'>";
							row += "<tr ondblclick='editDataAnalysisDetails(\""+data[i].id+"\")'>";
	 ...
	 function editDataAnalysisDetails(id){

		 
			$.ajax({
			    type: "GET",
			    contentType: "application/json",
			    url: "fetchDataAnalysisDetailsList?id="+id,
			    data: {},
			    dataType: 'json',
			    cache: false,
			    success: function (data) {
					if (data != null) {
						for(var i=0; i<data.length; i++){
							 ...
							 $("#labourOrRateAnalysisName"+data[i].itemId).val(data[i].rateAnalysisName);
							 $("#rate"+data[i].itemId).val(data[i].rate);
							
							 var unitvalue='';
							/* $("#unitMaster"+data[i].itemId+" option").each(function() {
							  if($(this).text() == unitName) {
							    $(this).attr('selected', 'selected');  
							    unitvalue = this.value;
							  }else{
							 ...
							 unitvalue=data[i].unit;
							 $("#unitMaster"+data[i].itemId).val(unitvalue);
							 $("#purpose"+data[i].itemId).val(data[i].purpose);
							 $("#quantity"+data[i].itemId).val(data[i].quantity);
							 $("#dataAnlybtn"+data[i].itemId).val('Update');
							 $("#dataAnlybtn"+data[i].itemId).attr("onclick","updateDataAnalysisData("+data[i].itemId+","+id+")");
						}
					};
			    }
		 ...
		 function redirectAbstractToMeasurement(id){
			...
			function showHistory(divId)
			{
				...
				$.ajax({
				    type: "GET",
				    contentType: "application/json",
				    url: "measurmentListHistory?id="+divId,
				    data: {},
				    dataType: 'json',
				    cache: false,
				    success: function (data) {
			...
			function showLabourDataHistory(divId)
			{
				...
				$.ajax({
				    type: "GET",
				    contentType: "application/json",
				    url: "labourListHistory?id="+divId,
				    data: {},
				    dataType: 'json',
				    cache: false,
				    success: function (data) {
