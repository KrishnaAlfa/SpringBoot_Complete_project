...
$(document)
		.ready(
				function() {
					...
					/*
					 * function checkBioDataConditions(){ var index =
					 * commonCounter;
					 * 
					 * var applicationTeamRole =
					 * $("#applicationTeamRole"+index).val(); var bioDataFile =
					 * $("#bioDataFile"+index).val(); var
					 * applicationTeamNationality =
					 * $("#applicationTeamNationality"+index).val(); var
					 * teamForeign =
					 * $("input[name='teamList[0].teamForeign']:checked").val();
					 * 
					 * if(applicationTeamRole == "Director" && bioDataFile ==
					 * ""){ alert("Upload bio data file"); }else
					 * if(applicationTeamRole == "CoDirector" && bioDataFile ==
					 * ""){ alert("Upload bio data file"); }else
					 * if(applicationTeamRole == "Director" &&
					 * applicationTeamNationality != "India"){ alert("Director
					 * should be Indian."); }else if(applicationTeamRole !=
					 * "Director" && applicationTeamRole != "CoDirector" &&
					 * applicationTeamNationality != "India" && bioDataFile ==
					 * ""){ alert("Bio data is mandatory as "
					 * +applicationTeamRole + " is not Indian"); }else
					 * if(teamForeign == 2 && applicationTeamNationality ==
					 * "India"){ alert("Foreign Collaboration should not be
					 * Indian"); }else{ $("input").attr("required", false);
					 * document.forms["form4"].submit();
					 * 
					 * $("#teamForm") .submit( function(e) { e.preventDefault();
					 * 
					 * var url = "teamInsertion"; $.ajax({ type : "POST", url :
					 * url, data : new FormData(this), processData : false,
					 * contentType : false, cache : false, success :
					 * function(data) { if(data != null){
					 * $("#successdiv").show("slow"); alert(data.length); if
					 * (data != null) { $('#addTeamList').empty(); var count =
					 * 1; for(var i=0; i<data.length; i++){ $('#addTeamList')
					 * .append('<tr>'+ '<td>'+data[i].applicationTeamDepartment+'</td>'+ '<td>'+data[i].applicationTeamRole+'</td>'+ '<td>'+data[i].applicationTeamName+'</td>'+ '<td>'+data[i].applicationTeamMobile+'</td>'+ '<td>'+data[i].applicationTeamEmail+'</td>'+ '<td>'+data[i].applicationTeamNationality+'</td>'+ '<td>'+data[i].applicationTeamAddress+'</td>'+ '<td><a
					 * href="/eASI/document/admin/download-'+data[i].applicationTeamId+'-bioData1">download</a></td>'+ '<td><a
					 * href="/eASI/document/admin/download-'+data[i].applicationTeamId+'-visa1">download</a></td>'+ '<td><a
					 * href="/eASI/document/admin/download-'+data[i].applicationTeamId+'-passport1">download</a></td>'+ '</tr>');
					 * commonCounter = count; $("#counter").val(++count); };
					 * 
					 *  };
					 *  } },beforeSend: function () { $("#commonLodder").show(); },
					 * complete: function () { waitingDialog.hide();
					 * $("#commonLodder").hide(); }, error :
					 * function(request,error) { alert("Something went wrong"); }
					 * }); });
					 * 
					 *  } }
					 */
										...
										$('#addRow1')
												.append(
														'<tr id="trow'
																...
																+ '].name" id="name'
																+ counts
																+ '" required="required" placeholder="" type="text"></td>'
																+ '<td><input name="sin['
																+ counts
					...
					$("#addMore5")
							.click(
									function() {
										...
										$('#addRow')
												.append(
														'<div class="addTeamCollaboration" id="collaborationDiv'
																+ count
																+ '">'
																+ '<div class="row">'
																+ '<div class="select">'
																...
																+ '<input  value="1" name="teamList['
																+ count
																+ '].teamForeign" type="radio"'
																+ ' id="teamForeign'
																+ count
																+ '" class="teamForeignCheck"> <span'
																+ 'class="lbl">Team Details</span>'
																+ '</label>'
																+ '</div>'
																+ '<div class="col-md-3">'
																+ '<label class="radioLabel"> '
																+ '<input checked="checked" name="teamList['
																+ count
																+ '].teamForeign" type="radio" value="2"'
																+ 'id="teamForeign'
																+ count
																+ '" class="teamForeignCheck"> <span class="lbl">Foreign'
																+ 'Collaboration</span>'
																+ '</label>'
																+ '</div>'
																+ '<i class="fa fa-remove" style="float: right;font-size: large;" title="Delete" onclick="deleteDiv(collaborationDiv'
																+ count
																+ ')"></i>'
																+ '</div>'
																+ '</div>'
																...
																+ '</span> <input name="teamList['
																+ count
																+ '].applicationTeamName"'
																+ 'id="applicationTeamName'
																+ count
																+ '" required="required"'
																...
																+ '<span style="color: red">*</span>'
																+ '</span> <select name="teamList['
																+ count
																+ '].applicationTeamRole"'
																+ 'id="applicationTeamRole'
																+ count
																+ '" required="required">'
																...
																+ '<textarea name="teamList['
																+ count
																+ '].applicationTeamAddress"'
																+ 'id="applicationTeamAddress'
																+ count
																+ '" maxlength="200"'
																								...
																								$(
																										...
																										.append(
																												'<tr>'
																														...
																														+ '-bioData1"><i class="fa fa-arrow-circle-o-down"></i></a></td>'
																														+ '<td><a href="/eASI/document/admin/download-'
																														+ data[i].applicationTeamId
																														+ '-visa1" class="visa1'
																														+ data[i].applicationTeamId
